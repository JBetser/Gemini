...
class BinanceClientDefaultFactory(BinanceClientFactory):
    def __init__(self, symbol):
        super(BinanceClientDefaultFactory, self).__init__(BinanceSocketManager.STREAM_URL + 'ws/' + symbol.lower() + '@depth' + BinanceSocketManager.WEBSOCKET_DEPTH_10)
...
class BinanceSocketManager(threading.Thread):
    ...
    def _start_socket(self, path, callback, prefix='ws/', custom_factory = None):
        if path in self._conns:
            return False

        factory_url = self.STREAM_URL + prefix + path
        factory = BinanceClientFactory(factory_url) if custom_factory is None else custom_factory
    ...
    def start_depth_socket(self, symbol, callback, depth=WEBSOCKET_DEPTH_1, custom_factory=None):
        ...
        return self._start_socket(socket_name, callback, custom_factory=custom_factory)
